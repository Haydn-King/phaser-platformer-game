...
class Collectables extends Phaser.Physics.Arcade
  .StaticGroup {
  constructor(scene) {
  ...
  addFromLayer(layer) {
    const { score: defaultScore, type } =
      this.mapProperties(layer.properties);

    layer.objects.forEach((collectableO) => {
      const collectable = this.get(
        collectableO.x,
        collectableO.y,
        type
      );
      const props = this.mapProperties(
        collectableO.properties
      );
