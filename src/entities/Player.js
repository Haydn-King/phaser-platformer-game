...
class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  init() {
    ...
    this.projectileSound = this.scene.sound.add(
      "projectile-launch",
      { volume: 0.2 }
    );
    ...
    this.swipeSound = this.scene.sound.add("swipe", {
      volume: 0.2,
    });
  ...
  update() {
    ...
    if (
      this.isPlayingAnims("throw") ||
      this.isPlayingAnims("slide")
    ) {
      return;
    }
  ...
  takesHit(source) {
    ...
    this.health -=
      source.damage || source.properties.damage || 0;
    if (this.health <= 0) {
      EventEmitter.emit("PLAYER_LOOSE");
      return;
    }
