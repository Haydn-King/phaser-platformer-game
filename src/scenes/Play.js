...
class Play extends Phaser.Scene {
  ...
  create({ gameStatus }) {
    ...
    this.playBgMusic();
    this.collectSound = this.sound.add("coin-pickup", {
      volume: 0.2,
    });
    ...
    const collectables = this.createCollectables(
      layers.collectables
    );
  ...
  playBgMusic() {
    ...
    this.sound
      .add("theme", { loop: true, volume: 0.03 })
      .play();
  }
  ...
  createLayers(map) {
    ...
    map
      .createStaticLayer("distance", tilesetBg)
      .setDepth(-12);
    ...
    platformsColliders.setCollisionByProperty({
      collides: true,
    });
  ...
  createBG(map) {
    const bgObject =
      map.getObjectLayer("distance_bg").objects[0];
  ...
  createCollectables(collectableLayer) {
    const collectables = new Collectables(this).setDepth(
      -1
    );
  ...
  setupFollowupCameraOn(player) {
    const { height, width, mapOffset, zoomFactor } =
      this.config;
  ...
  getPlayerZones(playerZonesLayer) {
    ...
    return {
      start: playerZones.find(
        (zone) => zone.name === "startZone"
      ),
      end: playerZones.find(
        (zone) => zone.name === "endZone"
      ),
    };
  }
  ...
  update() {
    this.spikesImage.tilePositionX =
      this.cameras.main.scrollX * 0.3;
    this.skyImage.tilePositionX =
